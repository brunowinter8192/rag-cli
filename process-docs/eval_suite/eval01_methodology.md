...
- **RAGAS (Evaluation Framework):** prescribes context_recall + context_precision as primary retrieval metrics; binary relevance via LLM-judge is valid for small GT sets. Ground truth at document-level (expected_documents) matches the RAGAS pattern. LLM-judge not needed here — relevance is by construction.
...
- **Methodology rationale + schema migration process:** binary relevance by construction, why LLM-judge is not needed, why doc-level approximation is wrong; Q1–Q3 sample established primary chunk identification, additional relevant chunk selection, cross-collection chunk_index lookup methodology.
...
- **Statistical significance:** sweep comparisons need paired t-test or Wilcoxon to distinguish real gains from noise. Currently reported only as raw averages. Needed before any config decision on alpha, top_k, or mode.
