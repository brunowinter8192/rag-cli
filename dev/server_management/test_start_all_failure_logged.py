...
def test_start_all_reports_and_logs_failed_starts(workdir: Path) -> None:
    server_cli = load_rag("server_cli")
    results = server_cli.start_all()
    ...
    for handler in server_cli.logger.handlers:
        handler.flush()
    log_text = (workdir / "src" / "rag" / "logs" / "server_cli.log").read_text()
    for name in results:
